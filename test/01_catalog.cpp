...
TEST_CASE(catalog_relation) {

    {
        ...
        optional<Relation> rt = cm.get_relation("test_table");
        if (rt)
            cm.remove_relation("test_table");
...

    {
        ...
        optional<Relation> rel = cm.get_relation("test_table");
        assert(!rel, "Relation not saved.");
        ...
        auto &f1 = fields[1];
        assert(f1.name == "name" && !f1.unique && f1.type.tag == Type::Tag::CHAR && f1.type.char_len == 20 &&
                   f1.offset == 4,
               "field info");
        auto &f2 = fields[2];
        assert(f2.name == "contact" && !f2.unique && f2.type.tag == Type::Tag::CHAR && f2.type.char_len == 20 &&
                   f2.offset == 24,
               "field info");
    }
}
...
TEST_CASE(catalog_index) {
...

    {
        ...
        optional<IndexLocation> il = cm.get_index_location("test_my_index");
        assert(il.has_value(), "index not found");
...

    {
        ...
        cm.remove_index("test_my_index");
        assert(!cm.get_index_location("test_my_index"), "not deleted");
    }
}