...
static void check_name(const string &name) {
    ...
    if (name.length() >= NAME_LENGTH)
        throw logic_error("Name exceeded maximum length.");
}
...
void CatalogManager::set_index_rel(const string &rel_name, int field_index, bool use_index) {
    ...
    if (block < 0)
        throw logic_error("Relation does not exist");

    BlockGuard bg_rel(block_mgr, Files::catalog(), block);
    auto *file_rel = bg_rel.addr<RelationData>();

    if (field_index < 0 || field_index >= file_rel->field_count)
        return;

    FieldData &field = file_rel->fields[field_index];
    if (field.has_index != use_index) {
        if (use_index && !field.unique)
            throw logic_error("The field must be unique.");
...
void CatalogManager::add_relation(const Relation &relation) {
    ...
    if (block >= 0)
        throw logic_error("Relation name duplicated.");

    // 从relation总表中找一个空位
    // TODO free_block这个名字不太好
    int free_block = file_db->get_free_block();
    if (free_block < 0)
        throw logic_error("Exceeded maximum relations.");
...
void CatalogManager::add_index(const string &rel_name, const string &field_name, const string &index_name) {
    ...
    if (free_index < 0)
        throw logic_error("Exceeded maximum indexes");
    ...
    if (!found)
        throw logic_error("This field does not exist");
