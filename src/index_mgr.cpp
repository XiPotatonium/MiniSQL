...
tree_information IndexManager::get_information(const Relation &rel, int field_index, const string &scheme) {
    if (block_mgr->file_blocks(scheme) == 0) {
        ...
        int degree = (BLOCK_SIZE - (sizeof(bool) * 2 + 2 * sizeof(int))) /
                     (rel.fields[field_index].type.length() + sizeof(int) + sizeof(RecordPosition));
        head->degree = degree;
        head->root = -1;
...
int IndexManager::get_new_block(const string &scheme) {
    block_mgr->file_append_block(scheme);
    int new_block = block_mgr->file_blocks(scheme) - 1;
    return new_block;
}

void IndexManager::insert_value(const string &scheme, Type type, const Value &value, int degree, int num, int pos) {
...
void IndexManager::insert_block(const string &scheme, Type type, int block_index, int degree, int num, int pos) {
    int offset = sizeof(treenode) + degree * type.length() + pos * sizeof(int);
    BlockGuard bg(block_mgr, scheme, num);
    *bg.addr<int>(offset) = block_index;
}

void IndexManager::shift_value(const string &scheme, Type type, int degree, int num, int pos, int direction) {
    Value temp_value;
    BlockGuard bg(block_mgr, scheme, num);
    if (direction > 0)
        for (int i = degree - 2; i >= pos; i--) {
...
RecordPosition IndexManager::get_record(const string &scheme, Type type, int num, int degree, int pos) {
    BlockGuard bg(block_mgr, scheme, num);
...
void IndexManager::shift_record(const string &scheme, Type type, int degree, int num, int pos, int direction) {
...
void IndexManager::shift_block(const string &scheme, Type type, int degree, int num, int pos, int direction) {
    int temp_block;
    if (direction > 0)
        for (int i = degree; i >= pos; i--) {
...
int IndexManager::get_block(const string &scheme, Type type, int num, int degree, int pos) {
    BlockGuard bg(block_mgr, scheme, num);
...
void IndexManager::fresh_information(string &scheme, Type type, int num, int degree, Value old_value, Value new_value) {
    ...
    Value temp_value;
    temp_value = get_value(bg, scheme, type, num, degree, i);
    if (temp_value.greater_than(old_value, type))
        return;
    for (i = 0; i < tree_info->size; i++) {
        temp_value = get_value(bg, scheme, type, num, degree, i);
        if (Value::cmp(type, old_value, temp_value) == 0)
            break;
    }
    if (i == tree_info->size)
        throw "Why not find father_info";
    insert_value(scheme, type, new_value, degree, num, i);
}

Value IndexManager::find_min(const string &scheme, Type type, int num, int degree) {
    BlockGuard bg(block_mgr, scheme, num);
    treenode *tree_info = bg.addr<treenode>();
...
void IndexManager::work(const string &scheme, const Relation &rel, int field_index, const Value &value,
                        RecordPosition record_pos, int num, int degree) {
        ...
        if (tree_info->size == degree) {
            ...
            if (tree_info->Is_root == false) {
                ...
                for (i = 0; i < fa_head->size; i++) {
                    ...
                    if (temp_value.greater_than(min_value, type))
                        break;
                }
    ...
    // �����Ҷ�ӽڵ�
        ...
        if (tree_info->size == degree) {
            ...
            if (tree_info->Is_root == false) {
                ...
                for (i = 0; i < fa_head->size; i++) {
                    temp_value = get_value(fa_bg, scheme, type, tree_info->fa, degree, i);
                    if (temp_value.greater_than(new_value, type))
                        break;
                }
                shift_value(scheme, type, degree, tree_info->fa, i, 1);
...
void IndexManager::add_item(const Relation &rel, int field_index, const Value &value, RecordPosition record_pos) {
    ...
    if (block_mgr->file_blocks(scheme) == 0) {
        ...
        tree_information *head = bg_init.addr<tree_information>();
        // degree = {Is_root + Is_leaf + fa + size} + data[degree] + block_pos[degree] + rec_address[degree]
        int degree = (BLOCK_SIZE - (sizeof(treenode))) /
                     (rel.fields[field_index].type.length() + sizeof(int) + sizeof(RecordPosition));
        head->degree = degree;
        head->root = -1;
        bg_init.set_modified();
